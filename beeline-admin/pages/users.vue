<template>
  ...
  <div class="row" ng-if="user.id">
    <div class="col-sm-3">
      <div>
        ...
        <div class="row">
          ...
          <div class="col-sm-12">
            <a ui-sref="^.credits({userId: user.id})"><button class="btn btn-primary btn-block btm-offset" ng-disabled="!user">Credits for this month</button></a>
          </div>
          <div class="col-sm-12">
    ...
    <div class="col-sm-9" ng-if="user.id && showRouteCreditHistory">
      <h2>History of '{{showRouteCreditHistory.tag}}'</h2>
      <route-credit-history-viewer user-id="user.id"
        credit-tag="showRouteCreditHistory.tag"
        final-balance="showRouteCreditHistory.balance"
        company-id="companyId">
      </route-credit-history-viewer>
    </div>
    <div class="col-sm-9" ng-if="!companyId">
    ...
    <div class="col-sm-9" ng-if="companyId">
      ...
      <table class="table">
        ...
        <tbody ng-repeat="credit in routeCredits">
          <tr ng-repeat="route in credit.routes">
            ...
            <td ng-if="$index == 0" rowspan="{{ credit.routes.length }}" class="width-limit-xs">{{ credit.balance | currency }}</td>
            <td>
              <a ui-sref="^.trips({routeId: route.id, action: 'route'})" class="route-label">{{ route.label }}</a>
            </td>
